...
pc PC(.en(complete_it),.clk(CLK),.rstb(RSTB),.pco(PC_OUT[7:0]));
//--IF-----------------------------------------------------------------
...
pipe_if_id_P3 IF_ID(.rstb(RSTB),.clk(CLK),.en(complete_it),.id_xa(ID_XA),.id_ra(ID_RA),
...
register_file REG_FILE(.r1a(4'b0000),.r2a(4'b0000),.r3a(ID_XA),.wa(complete_it),.wd(complete_it),
                       .reg_write(complete_it),.r1d(),.r2d(),.r3d(ID_XD),.clk(CLK));
...
assign ID_MOV_OUT  = (         ) & (ID_MOV);
assign ID_SUB3_OUT = (         ) & (ID_SUB3);
assign ID_ADD4_OUT = (         ) & (ID_ADD4);
assign ID_ADD1_OUT = (         ) & (ID_ADD1);
...
pipe_reg2 ID_EX1(.rstb(RSTB),.clk(CLK),.en(complete_it),
...
assign PRIORITY =  						; // Priority between forwarding help from the EX2 stage and the WB stage
assign FORW1 =   						; // Decide whether to receive any forwarding help or carry EX1_XD
assign EX1_PRIO_XD = PRIORITY ?  		; // EX1_PRIO_XD = The output the mux controlled by the selection line PRIORITY
assign EX1_ADDER_IN = FORW1 ?  			; // EX1_ADDER_IN = The output the forwarding mux which is the input to the adder in EX1
assign EX1_ADDER_OUT = EX1_ADDER_IN + (-3); // Subtract 3
assign SKIP1 =                          ; // Decide whether to skip the subtractor result
assign EX1_XD_OUT = SKIP1 ?             ; // mux to skip the subtractor result, (i.e. select between EX1_ADDER_IN  EX1_ADDER_OUT) Note: EX1_XD_OUT = Data going out of EX1
...
// 		vec4_in1,vec4_in2,vec4_out1,vec4_out2,
// 		bit_in1,bit_in2,bit_in3,bit_in4,bit_in5,bit_in6,bit_out1,bit_out2,bit_out3,bit_out4,bit_out5,bit_out6,instr_in,instr_out);
pipe_reg2 EX1_EX2(.rstb(RSTB),.clk(CLK),.en(complete_it),
...
assign FORW2 =                               ;  // Is it a case of forwarding date from the WB stage
assign EX2_ADDER_IN = FORW2 ?       :        ;  // select between EX2_XD and WB_BD
assign EX2_ADDER_OUT = EX2_ADDER_IN + (+4);     // Add 4
assign SKIP2 =                               ;  // Is it a case of skipping the ADD4 operation
assign EX2_XD_OUT = SKIP2 ?         :        ;  // Skip add4 result if needed (i.e. select between EX2_ADDER_IN  EX2_ADDER_OUT)
assign EX2_WRITE =                           ;  // Produce one signal out of EX2_MOV,EX2_SUB3,EX2_ADD4,EX2_ADD1
